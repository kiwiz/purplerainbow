...
headers = {"X-Estimize-Key" : "38704631c5b84c02a4bb900e", "content-type" : "application/json"}
...
def request_release_consensus(release_id):
    """
    Overview: Returns the current consensus as well as the consensus history of the specified release.
    Required params: None
    Optional params: None
    Returns: An object that has wallstreet and estimize properties, that each include eps and revenue properties, with the following attributes:
    ...
    """
    function_url = url + "/releases/%s/consensus" % (release_id)
    req = requests.get(function_url, headers=headers)
    if req.status_code != 200 : return None
    return req.json()
